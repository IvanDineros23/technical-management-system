...
@section('content')
    <div x-data="assignmentsPage()" @keydown.window="handleKeydown($event)">
        ...
        <div class="mb-6 flex flex-col gap-3 sm:flex-row sm:gap-2 sm:items-center">
            <!-- Status Filter -->
            <div class="flex items-center gap-2">
                <select x-model="selectedStatus" @change="filterAssignments()" 
                        class="w-full sm:w-auto px-3 sm:px-4 py-2 border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-800 text-gray-900 dark:text-white text-xs sm:text-sm font-medium hover:border-blue-400 dark:hover:border-blue-500 focus:border-blue-500 dark:focus:border-blue-500 transition-colors">
            ...
            <div class="flex-1 relative">
                <div class="relative">
                    <svg class="w-4 sm:w-5 h-4 sm:h-5 text-gray-400 absolute left-3 top-1/2 transform -translate-y-1/2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <!-- Results Count -->
            <div class="flex items-center px-3 sm:px-4 py-2 bg-gray-100 dark:bg-gray-700 rounded-lg text-xs sm:text-sm font-medium text-gray-700 dark:text-gray-300 whitespace-nowrap">
            ...
            @if($assignments->count() > 0)
                <div class="space-y-4">
                    ...
                    <div id="job-{{ $job->id }}"
                         ...
                         class="p-3 sm:p-4 border border-gray-200 dark:border-gray-700 rounded-xl hover:shadow-md transition-shadow">
                        <div class="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-3 sm:gap-0">
                            <div class="flex-1">
                                <div class="flex flex-wrap items-center gap-2 sm:gap-3 mb-2">
                                    <h3 class="text-base sm:text-lg font-semibold text-gray-900 dark:text-white">{{ $job->job_order_number }}</h3>
                                ...
                                <p class="text-xs sm:text-sm text-gray-600 dark:text-gray-400 mb-2">{{ $job->description ?? 'No description' }}</p>
                                <div class="flex flex-wrap items-center gap-2 sm:gap-4 text-xs sm:text-sm text-gray-500 dark:text-gray-400">
                                    <span class="flex items-center gap-1">
                                        ...
                                        <span class="truncate">{{ $job->customer->name ?? 'N/A' }}</span>
                                    </span>
                                    <span class="flex items-center gap-1">
                                        <svg class="w-3 sm:w-4 h-3 sm:h-4 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                            ...
                            <div class="flex flex-wrap gap-2 sm:flex-col sm:gap-2 sm:ml-4 w-full sm:w-auto">
                                ...
                                <button type="button" disabled
                                        class="flex-1 sm:flex-none px-3 sm:px-4 py-2 bg-amber-100 text-amber-800 rounded-lg text-xs sm:text-sm font-medium cursor-not-allowed whitespace-nowrap">
                                    Pending Review
                                </button>
        ...
        <!-- Job Details Modal -->
        <div x-show="showModal" x-cloak x-transition:enter="ease-out duration-300" x-transition:enter-start="opacity-0" x-transition:enter-end="opacity-100" x-transition:leave="ease-in duration-200" x-transition:leave-start="opacity-100" x-transition:leave-end="opacity-0" 
            ...
            <div x-show="showModal" x-transition:enter="ease-out duration-300" x-transition:enter-start="opacity-0 translate-y-4 sm:translate-y-0 sm:scale-95" x-transition:enter-end="opacity-100 translate-y-0 sm:scale-100" x-transition:leave="ease-in duration-200" x-transition:leave-start="opacity-100 translate-y-0 sm:scale-100" x-transition:leave-end="opacity-0 translate-y-4 sm:translate-y-0 sm:scale-95"
                 ...
                 @click.stop>
                <!-- Modal Header -->
                <div class="sticky top-0 bg-white dark:bg-gray-800 border-b border-gray-200 dark:border-gray-700 p-4 sm:p-6 flex items-center justify-between">
                    <div>
                        <h2 class="text-lg sm:text-2xl font-bold text-gray-900 dark:text-white" x-text="selectedJob?.jobNumber"></h2>
                        <p class="text-xs sm:text-sm text-gray-600 dark:text-gray-400 mt-1">Complete job details</p>
                    </div>
                ...
                <div class="p-4 sm:p-6 space-y-4 sm:space-y-6">
                    ...
                    <div class="grid grid-cols-2 gap-3 sm:gap-4">
                        <div>
                            <p class="text-xs font-semibold text-gray-600 dark:text-gray-400 uppercase mb-1 sm:mb-2">Status</p>
                            <p class="text-sm sm:text-lg font-semibold text-gray-900 dark:text-white" x-text="selectedJob?.status?.replace(/_/g, ' ')"></p>
                        </div>
                        <div>
                            <p class="text-xs font-semibold text-gray-600 dark:text-gray-400 uppercase mb-1 sm:mb-2">Priority</p>
                            <span x-show="selectedJob?.priority === 'urgent'" class="inline-block px-2 sm:px-3 py-1 text-xs sm:text-sm font-bold rounded-full bg-red-100 text-red-800 dark:bg-red-900/30 dark:text-red-300">
                                URGENT
                            </span>
                            <span x-show="selectedJob?.priority !== 'urgent'" class="text-sm sm:text-lg font-semibold text-gray-900 dark:text-white" x-text="selectedJob?.priority?.replace(/_/g, ' ')"></span>
                        </div>
                    </div>

                    <!-- Description -->
                    <div>
                        <p class="text-xs font-semibold text-gray-600 dark:text-gray-400 uppercase mb-1 sm:mb-2">Description</p>
                        <p class="text-sm sm:text-base text-gray-700 dark:text-gray-300" x-text="selectedJob?.description"></p>
                    </div>

                    <!-- Customer & Date Row -->
                    <div class="grid grid-cols-2 gap-3 sm:gap-4">
                        <div>
                            <p class="text-xs font-semibold text-gray-600 dark:text-gray-400 uppercase mb-1 sm:mb-2">Customer</p>
                            <p class="text-sm sm:text-base text-gray-900 dark:text-white" x-text="selectedJob?.customer"></p>
                        </div>
                        <div>
                            <p class="text-xs font-semibold text-gray-600 dark:text-gray-400 uppercase mb-1 sm:mb-2">Created Date</p>
                            <p class="text-sm sm:text-base text-gray-900 dark:text-white" x-text="selectedJob?.createdAt"></p>
                        </div>
                    </div>
                    ...
                    <div class="grid grid-cols-2 gap-3 sm:gap-4">
                        <div>
                            <p class="text-xs font-semibold text-gray-600 dark:text-gray-400 uppercase mb-1 sm:mb-2">Location</p>
                            <p class="text-sm sm:text-base text-gray-900 dark:text-white" x-text="selectedJob?.location"></p>
                        </div>
                        <div>
                            <p class="text-xs font-semibold text-gray-600 dark:text-gray-400 uppercase mb-1 sm:mb-2">Equipment</p>
                            <p class="text-sm sm:text-base text-gray-900 dark:text-white" x-text="selectedJob?.equipment"></p>
                        </div>
                    </div>

                    <!-- Notes -->
                    <div>
                        <p class="text-xs font-semibold text-gray-600 dark:text-gray-400 uppercase mb-1 sm:mb-2">Notes</p>
                        <p class="text-sm sm:text-base text-gray-700 dark:text-gray-300" x-text="selectedJob?.notes"></p>
                    </div>

                    <!-- Rejection Notes -->
                    <div x-show="selectedJob?.status === 'rejected'" class="rounded-lg border border-red-200 dark:border-red-700 bg-red-50 dark:bg-red-900/20 p-3 sm:p-4">
                        <p class="text-xs font-semibold text-red-700 dark:text-red-300 uppercase mb-1 sm:mb-2">Tech Head Rejection Reason</p>
                        <p class="text-xs sm:text-sm text-red-800 dark:text-red-200" x-text="selectedJob?.reviewNotes || 'No rejection reason provided.'"></p>
                    </div>
                    ...
                    <div class="flex flex-col sm:flex-row sm:justify-between gap-3">
                        <div x-show="selectedJob" class="flex flex-col sm:flex-row gap-2">
                            <a x-show="selectedJob" :href="selectedJob ? `{{ route('technician.job-details', ['id' => '__ID__']) }}`.replace('__ID__', selectedJob.id) : '#'"
                               class="px-3 sm:px-4 py-2 bg-blue-600 text-white rounded-lg text-xs sm:text-sm font-medium hover:bg-blue-700 transition-colors inline-flex items-center justify-center sm:justify-start gap-2">
                                <svg class="w-3 sm:w-4 h-3 sm:h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"/>
                                </svg>
                                <span class="hidden sm:inline">View Full Details</span>
                                <span class="sm:hidden">Details</span>
                            </a>
                        </div>
                        <div class="flex flex-col sm:flex-row gap-2">
                            <button @click="closeModal()"
                                    class="px-3 sm:px-4 py-2 bg-gray-100 dark:bg-gray-700 text-gray-700 dark:text-gray-300 rounded-lg text-xs sm:text-sm font-medium hover:bg-gray-200 dark:hover:bg-gray-600 transition-colors">
                                Close
                            </button>
                            <template x-if="selectedJob?.status === 'assigned'">
                                <button @click="openConfirmStart(selectedJob.id); closeModal();"
                                        class="px-3 sm:px-4 py-2 bg-blue-600 text-white rounded-lg text-xs sm:text-sm font-medium hover:bg-blue-700 transition-colors whitespace-nowrap">
                                    Start Work
                                </button>
                            </template>
                            <template x-if="selectedJob?.status === 'in_progress'">
                                <button @click="openConfirmPause(selectedJob.id)"
                                        class="px-3 sm:px-4 py-2 bg-orange-600 text-white rounded-lg text-xs sm:text-sm font-medium hover:bg-orange-700 transition-colors whitespace-nowrap">
                                    Pause
                                </button>
                            </template>
                            <template x-if="selectedJob?.status === 'in_progress'">
                                <button @click="openConfirmComplete(selectedJob.id);"
                                        class="px-3 sm:px-4 py-2 bg-green-600 text-white rounded-lg text-xs sm:text-sm font-medium hover:bg-green-700 transition-colors whitespace-nowrap">
                                    Complete
                                </button>
                            </template>
                            <template x-if="selectedJob?.status === 'rejected'">
                                <a x-show="selectedJob" :href="selectedJob ? `{{ route('technician.job-details', ['id' => '__ID__']) }}`.replace('__ID__', selectedJob.id) : '#'"
                                   class="px-3 sm:px-4 py-2 bg-red-600 text-white rounded-lg text-xs sm:text-sm font-medium hover:bg-red-700 transition-colors inline-flex items-center justify-center whitespace-nowrap">
                                    Revise & Resubmit
                                </a>
                            </template>
                        </div>
        ...
        <!-- Confirmation Modal -->
        <div x-show="showConfirmModal" x-cloak x-transition:enter="ease-out duration-300" x-transition:enter-start="opacity-0" x-transition:enter-end="opacity-100" x-transition:leave="ease-in duration-200" x-transition:leave-start="opacity-100" x-transition:leave-end="opacity-0" 
             class="fixed inset-0 bg-gray-500 dark:bg-gray-900 bg-opacity-75 dark:bg-opacity-75 z-50 flex items-center justify-center p-4 transition-opacity"
            ...
            <div x-show="showConfirmModal" x-transition:enter="ease-out duration-300" x-transition:enter-start="opacity-0 translate-y-4 sm:translate-y-0 sm:scale-95" x-transition:enter-end="opacity-100 translate-y-0 sm:scale-100" x-transition:leave="ease-in duration-200" x-transition:leave-start="opacity-100 translate-y-0 sm:scale-100" x-transition:leave-end="opacity-0 translate-y-4 sm:translate-y-0 sm:scale-95"
                 class="bg-white dark:bg-gray-800 rounded-[20px] shadow-xl max-w-md w-full border border-gray-200 dark:border-gray-700 transform transition-all sm:align-middle p-4 sm:p-6"
                 @click.stop>
                
                <!-- Icon -->
                <div class="flex items-center justify-center w-10 sm:w-12 h-10 sm:h-12 mx-auto mb-3 sm:mb-4 rounded-full"
                     :class="{
                    ...
                    <svg x-show="confirmAction === 'start'" class="w-5 sm:w-6 h-5 sm:h-6 text-blue-600 dark:text-blue-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M14.752 11.168l-3.197-2.132A1 1 0 0010 9.87v4.263a1 1 0 001.555.832l3.197-2.132a1 1 0 000-1.664z"/>
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M21 12a9 9 0 11-18 0 9 9 0 0118 0z"/>
                    </svg>
                    <svg x-show="confirmAction === 'complete'" class="w-5 sm:w-6 h-5 sm:h-6 text-green-600 dark:text-green-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z"/>
                    </svg>
                    <svg x-show="confirmAction === 'pause'" class="w-5 sm:w-6 h-5 sm:h-6 text-orange-600 dark:text-orange-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <div class="flex flex-col-reverse sm:flex-row gap-2 sm:gap-3">
                    <button @click="closeConfirmModal()"
                            class="flex-1 px-3 sm:px-4 py-2 bg-gray-100 dark:bg-gray-700 text-gray-700 dark:text-gray-300 rounded-lg text-xs sm:text-sm font-medium hover:bg-gray-200 dark:hover:bg-gray-600 transition-colors">
                        Cancel
                    </button>
                    <button @click="confirmAction === 'start' ? confirmStartJob() : (confirmAction === 'complete' ? confirmCompleteJob() : confirmPauseJob())"
                            ...
                            class="flex-1 px-3 sm:px-4 py-2 text-white rounded-lg text-xs sm:text-sm font-medium transition-colors">
