...
<div class="space-y-6" x-data="{ showRequest: false, showMyRequests: false }">
    ...
    <div class="bg-white dark:bg-gray-800 rounded-[20px] shadow-md border border-gray-200 dark:border-gray-700 p-6">
        <div class="flex items-center justify-between gap-4 mb-6">
            <div>
                <h3 class="text-base font-bold text-slate-900 dark:text-white">Equipment</h3>
                <p class="text-xs text-gray-500 dark:text-gray-400 mt-1">Browse and request available equipment</p>
            </div>
            <div class="flex items-center gap-3">
                <form method="GET" action="{{ route('technician.equipment') }}" class="flex items-center gap-2">
                    <div class="relative">
                        <svg class="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z"/>
                        </svg>
                        <input type="text" name="search" value="{{ $search ?? '' }}" placeholder="Search equipment..." class="pl-9 pr-4 py-2 text-sm rounded-lg border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-700 text-gray-900 dark:text-white focus:ring-2 focus:ring-indigo-500 w-52">
                    </div>
                    @if(!empty($search))
                    <a href="{{ route('technician.equipment') }}" class="text-xs text-gray-500 hover:text-gray-700 dark:text-gray-400 dark:hover:text-gray-200">Clear</a>
                    @endif
                </form>
                @if(isset($myRequests) && $myRequests->count() > 0)
                <button @click="showMyRequests=true" class="flex items-center gap-2 px-4 py-2 bg-gray-100 dark:bg-gray-700 hover:bg-gray-200 dark:hover:bg-gray-600 text-gray-700 dark:text-gray-300 rounded-lg text-sm font-medium transition-colors whitespace-nowrap">
                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2"/>
                    </svg>
                    My Requests
                    <span class="inline-flex items-center justify-center w-5 h-5 text-xs font-bold bg-indigo-600 text-white rounded-full">{{ $myRequests->count() }}</span>
                </button>
                @endif
                <button @click="showRequest=true" class="flex items-center gap-2 px-4 py-2 bg-indigo-600 hover:bg-indigo-700 text-white rounded-lg text-sm font-medium transition-colors whitespace-nowrap">
                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <div class="text-center py-10">
            ...
            <p class="text-gray-600 dark:text-gray-400">{{ !empty($search) ? 'No equipment found for "' . e($search) . '"' : 'No equipment available' }}</p>
        </div>
        @endif
    </div>
    ...
    >
        ...
        <div class="flex min-h-full items-center justify-center p-4">
            <div
                ...
                class="relative w-full max-w-4xl bg-white dark:bg-gray-800 rounded-[20px] shadow-xl border border-gray-200 dark:border-gray-700"
            >
                <div class="p-6">
                    <div class="flex items-center justify-between mb-5">
                        <div>
                            <h3 class="text-lg font-bold text-gray-900 dark:text-white">My Equipment Requests</h3>
                            <p class="text-xs text-gray-500 dark:text-gray-400 mt-0.5">Track the status of your equipment requests</p>
                        </div>
                        <button type="button" @click="showMyRequests=false" class="text-gray-400 hover:text-gray-600 dark:hover:text-gray-200">
                            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        ...
                        <table class="w-full">
                            ...
                            <tbody class="divide-y divide-gray-100 dark:divide-gray-700">
                                ...
                                <tr class="hover:bg-gray-50 dark:hover:bg-gray-700/50">
                                    <td class="py-3 text-sm font-medium text-gray-900 dark:text-white">{{ $req->equipment_name }}</td>
                                    <td class="py-3 text-sm text-gray-600 dark:text-gray-400 max-w-xs truncate">{{ $req->purpose }}</td>
                                    <td class="py-3 text-center">
                                        @if($req->jobOrder)
                                    ...
                                    <td class="py-3 text-sm text-gray-600 dark:text-gray-400 text-center">{{ $req->created_at->format('M d, Y') }}</td>
                                    <td class="py-3 text-sm text-gray-500 dark:text-gray-400 italic">{{ $req->admin_notes ?? '-' }}</td>
                                </tr>
                                @endforeach
                            </tbody>
                        </table>
            ...
            >
                <form method="POST" action="{{ route('technician.equipment.request') }}" class="p-6 space-y-4">
                    ...
                    <div class="flex items-center justify-between">
                        ...
                        <button type="button" @click="showRequest=false" class="text-gray-400 hover:text-gray-600 dark:hover:text-gray-200">
                            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"/>
                            </svg>
                        </button>
                    ...
                    <div x-data="{ selectMode: 'existing', eqName: '', eqId: '' }">
                        <div class="flex gap-3 mb-4">
                            <button type="button" @click="selectMode='existing'; eqName=''; eqId=''" :class="selectMode==='existing' ? 'bg-indigo-600 text-white' : 'bg-gray-100 dark:bg-gray-700 text-gray-700 dark:text-gray-300'" class="flex-1 py-2 rounded-lg text-sm font-medium transition-colors">Select from Inventory</button>
                            <button type="button" @click="selectMode='new'; eqName=''; eqId=''" :class="selectMode==='new' ? 'bg-indigo-600 text-white' : 'bg-gray-100 dark:bg-gray-700 text-gray-700 dark:text-gray-300'" class="flex-1 py-2 rounded-lg text-sm font-medium transition-colors">Enter Equipment Name</button>
                        </div>
                        ...
                        <div x-show="selectMode==='existing'">
                            ...
                            <select @change="eqId=$event.target.value; eqName=$event.target.selectedOptions[0].dataset.name" class="w-full px-3 py-2 rounded-lg border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-700 text-sm text-gray-900 dark:text-white">
                        ...
                        <div x-show="selectMode==='new'">
                            ...
                            <input type="text" x-model="eqName" placeholder="e.g., Oscilloscope Model XYZ" class="w-full px-3 py-2 rounded-lg border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-700 text-sm text-gray-900 dark:text-white" />
                        </div>
                    ...
                    <div>
                        ...
                        <textarea name="purpose" rows="3" required placeholder="Explain why you need this equipment..." class="w-full px-3 py-2 rounded-lg border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-700 text-sm text-gray-900 dark:text-white resize-none"></textarea>
                    </div>
                    ...
                    <div class="flex justify-end gap-3 pt-2">
                        <button type="button" @click="showRequest=false" class="px-4 py-2 bg-gray-200 dark:bg-gray-600 text-gray-800 dark:text-gray-200 rounded-lg text-sm font-medium hover:bg-gray-300 dark:hover:bg-gray-500 transition-colors">Cancel</button>
