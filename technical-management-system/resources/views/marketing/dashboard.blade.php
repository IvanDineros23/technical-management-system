    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-4">
            ...
            <div class="p-4 space-y-2">
                @forelse($recentJobOrders as $jobOrder)
                    <div class="flex items-center justify-between p-3 bg-slate-50 dark:bg-gray-700 rounded-xl hover:bg-slate-100 dark:hover:bg-gray-600 transition-colors cursor-pointer" @click='openJODetails(@json($jobOrder))'>
                        ...
                        @else
                            <span class="px-3 py-1 text-xs font-semibold bg-gray-100 dark:bg-gray-700 text-gray-700 dark:text-gray-300 rounded-full">{{ ucwords(str_replace('_', ' ', $jobOrder->status)) }}</span>
                        @endif
                    </div>
                @empty
        ...
        <div class="bg-white dark:bg-gray-800 rounded-[20px] shadow-md dark:shadow-gray-900/50 border border-gray-200 dark:border-gray-700">
            ...
            <div class="p-4 space-y-2">
                ...
                <a href="{{ route('marketing.customer-request-forms') }}" class="w-full flex items-center gap-3 p-3 bg-slate-50 dark:bg-gray-700 text-slate-700 dark:text-gray-200 rounded-xl hover:bg-slate-100 dark:hover:bg-gray-600 transition-colors">
                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"/>
                    </svg>
                    <span class="font-semibold">Generate Request Form</span>
                </a>
            </div>
        </div>
