...
<div class="space-y-6" x-data="{ showView: false, showEdit: false, showAdd: false, showDelete: false, showRequests: false, selectedItem: null, formData: { name: '', sku: '', category: '', quantity: 0, unit: 'units', min_level: 0, notes: '' } }">
    ...
    <div class="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        ...
        <div class="flex flex-col sm:flex-row gap-3">
            <button type="button" @click="showRequests = true" class="w-full sm:w-auto inline-flex justify-center items-center gap-2 px-4 py-2 bg-indigo-600 text-white rounded-lg hover:bg-indigo-700 transition-colors">
                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"/>
                </svg>
                View Requests
            </button>
            <button @click="showAdd = true; formData = { name: '', sku: '', category: '', quantity: 0, unit: 'units', min_level: 0, notes: '' }" class="w-full sm:w-auto inline-flex justify-center items-center gap-2 px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition-colors">
                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        @if($items->count() > 0)
            <div class="space-y-3 md:hidden p-4">
                @foreach($items as $item)
                    <div class="border border-gray-200 dark:border-gray-700 rounded-2xl p-4 bg-gray-50/80 dark:bg-gray-700/30">
                        <div class="flex items-start justify-between gap-3">
                            <div class="min-w-0">
                                <p class="text-xs uppercase tracking-wide text-gray-500 dark:text-gray-400">Item</p>
                            ...
                            <div class="text-right">
                                ...
                                <span class="inline-flex items-center px-2 py-1 text-[11px] font-medium rounded-full mt-2
                                    ...
                                    {{ $item->status === 'normal' ? 'bg-green-100 text-green-800 dark:bg-green-900/30 dark:text-green-200' : '' }}">
                        ...
                        <div class="mt-3 grid grid-cols-2 gap-3 text-sm">
                            <div>
                                ...
                                <p class="font-medium text-gray-900 dark:text-white">{{ $item->category ?? 'N/A' }}</p>
                            </div>
                            <div>
                        ...
                        <div class="mt-4 flex flex-wrap gap-2">
                            <button @click="selectedItem = @js($item); showView = true" class="px-3 py-2 rounded-lg bg-slate-100 dark:bg-gray-700 text-sm">View</button>
                            <button @click="selectedItem = @js($item); formData = { name: selectedItem.name, sku: selectedItem.sku, category: selectedItem.category, quantity: selectedItem.quantity, unit: selectedItem.unit, min_level: selectedItem.min_level, notes: selectedItem.notes }; showEdit = true" class="px-3 py-2 rounded-lg bg-blue-600 text-white text-sm">Edit</button>
                            <button @click="selectedItem = @js($item); showDelete = true" class="px-3 py-2 rounded-lg bg-rose-600 text-white text-sm">Delete</button>
                        </div>
                    </div>
                @endforeach
            </div>
        @else
            <div class="p-6 text-center text-sm text-gray-500 dark:text-gray-400">No inventory items found.</div>
        @endif
    ...
    <div x-show="showView" x-cloak class="fixed inset-0 z-50 overflow-y-auto" role="dialog" aria-modal="true" @keydown.escape.window="showView=false">
        <div class="flex items-start sm:items-center justify-center min-h-screen px-2 sm:px-4 pt-4 pb-6 sm:pb-20 text-center sm:block sm:p-0">
            <div x-show="showView"
            ...
            <span class="hidden sm:inline-block sm:align-middle sm:h-screen" aria-hidden="true">&#8203;</span>
              ...
              <div x-show="showView"
                 ...
                 x-transition:leave-end="opacity-0 translate-y-4 sm:translate-y-0 sm:scale-95"
                  class="inline-block align-bottom bg-white dark:bg-gray-800 rounded-2xl text-left overflow-hidden shadow-xl transform transition-all sm:align-middle sm:max-w-xl sm:w-full border border-gray-200 dark:border-gray-700 max-h-[90vh] overflow-y-auto">
        ...
        <div class="flex items-start sm:items-center justify-center min-h-screen px-2 sm:px-4 pt-4 pb-6 sm:pb-20 text-center sm:block sm:p-0">
            ...
            <span class="hidden sm:inline-block sm:align-middle sm:h-screen" aria-hidden="true">&#8203;</span>
              ...
              <div x-show="showEdit"
                 ...
                 x-transition:leave-start="opacity-100 translate-y-0 sm:scale-100"
                 x-transition:leave-end="opacity-0 translate-y-4 sm:translate-y-0 sm:scale-95"
                  class="inline-block align-bottom bg-white dark:bg-gray-800 rounded-2xl text-left overflow-hidden shadow-xl transform transition-all sm:align-middle sm:max-w-2xl sm:w-full border border-gray-200 dark:border-gray-700 max-h-[90vh] overflow-y-auto">
                 <form method="POST" :action="selectedItem ? '{{ url('tech-head/inventory') }}/' + selectedItem.id : '#'" class="p-4 sm:p-6 space-y-4">
                    @csrf
                    @method('PUT')
                    <div class="flex items-center justify-between">
                    ...
                    <div class="flex flex-col-reverse sm:flex-row sm:justify-end gap-3 pt-2">
                        <button type="button" @click="showEdit=false" class="w-full sm:w-auto px-4 py-2 rounded-lg border border-gray-300 dark:border-gray-600 text-gray-700 dark:text-gray-300">Cancel</button>
                        <button type="submit" class="w-full sm:w-auto px-4 py-2 rounded-lg bg-blue-600 text-white hover:bg-blue-700">Save</button>
                    </div>
                </form>
            </div>
        </div>
    ...
    <div x-show="showAdd" x-cloak class="fixed inset-0 z-50 overflow-y-auto" role="dialog" aria-modal="true" @keydown.escape.window="showAdd=false">
        <div class="flex items-start sm:items-center justify-center min-h-screen px-2 sm:px-4 pt-4 pb-6 sm:pb-20 text-center sm:block sm:p-0">
            ...
            <span class="hidden sm:inline-block sm:align-middle sm:h-screen" aria-hidden="true">&#8203;</span>
            <div x-show="showAdd" x-transition:enter="ease-out duration-300" x-transition:enter-start="opacity-0 translate-y-4 sm:translate-y-0 sm:scale-95" x-transition:enter-end="opacity-100 translate-y-0 sm:scale-100" x-transition:leave="ease-in duration-200" x-transition:leave-start="opacity-100 translate-y-0 sm:scale-100" x-transition:leave-end="opacity-0 translate-y-4 sm:translate-y-0 sm:scale-95" class="inline-block align-bottom bg-white dark:bg-gray-800 rounded-2xl text-left overflow-hidden shadow-xl transform transition-all sm:align-middle sm:max-w-2xl sm:w-full border border-gray-200 dark:border-gray-700 max-h-[90vh] overflow-y-auto">
                <form method="POST" action="{{ route('tech-head.inventory.store') }}" class="p-4 sm:p-6 space-y-4">
                    @csrf
    ...
    <div x-show="showRequests" x-cloak class="fixed inset-0 z-50 overflow-y-auto" role="dialog" aria-modal="true" @keydown.escape.window="showRequests=false">
        <div class="flex items-center justify-center min-h-screen px-4 pt-4 pb-20 text-center sm:block sm:p-0">
            <div x-show="showRequests" x-transition:enter="ease-out duration-300" x-transition:enter-start="opacity-0" x-transition:enter-end="opacity-100" x-transition:leave="ease-in duration-200" x-transition:leave-start="opacity-100" x-transition:leave-end="opacity-0" @click="showRequests=false" class="fixed inset-0 bg-gray-500 dark:bg-gray-900 bg-opacity-75 dark:bg-opacity-75 transition-opacity" aria-hidden="true"></div>
            <span class="hidden sm:inline-block sm:align-middle sm:h-screen" aria-hidden="true">&#8203;</span>
            <div x-show="showRequests" x-transition:enter="ease-out duration-300" x-transition:enter-start="opacity-0 translate-y-4 sm:translate-y-0 sm:scale-95" x-transition:enter-end="opacity-100 translate-y-0 sm:scale-100" x-transition:leave="ease-in duration-200" x-transition:leave-start="opacity-100 translate-y-0 sm:scale-100" x-transition:leave-end="opacity-0 translate-y-4 sm:translate-y-0 sm:scale-95" class="inline-block align-bottom bg-white dark:bg-gray-800 rounded-2xl text-left overflow-hidden shadow-xl transform transition-all sm:align-middle sm:max-w-5xl sm:w-full border border-gray-200 dark:border-gray-700">
                <div class="p-6">
                    <div class="flex items-center justify-between mb-4">
                        <div>
                            ...
                            <tbody class="divide-y divide-gray-200 dark:divide-gray-700">
                                ...
                                <tr class="hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-colors">
                                    ...
                                    <td class="px-4 py-3 text-sm space-x-2">
                                        @if($inventoryRequest->status === 'pending')
                                        <form method="POST" action="{{ route('tech-head.inventory.requests.update', $inventoryRequest) }}" class="inline">
                                            @csrf
                                            @method('PATCH')
