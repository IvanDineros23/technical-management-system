...
<div class="space-y-4 sm:space-y-6"
            ...
            <table class="w-full text-center">
                <thead>
                    <tr class="border-b border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-900">
                        ...
                        <th class="px-6 py-4 text-center text-sm font-semibold text-gray-900 dark:text-white">Description</th>
                        <th class="px-6 py-4 text-center text-sm font-semibold text-gray-900 dark:text-white">Users</th>
                ...
                <tbody class="divide-y divide-gray-200 dark:divide-gray-700">
                    ...
                    <tr class="hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-colors">
                        <td class="px-6 py-4 text-sm font-medium text-gray-900 dark:text-white text-center">{{ $role->name }}</td>
                        <td class="px-6 py-4 text-sm text-gray-600 dark:text-gray-400 text-center">{{ $role->description ?? 'N/A' }}</td>
                        <td class="px-6 py-4 text-sm font-medium text-gray-900 dark:text-white text-center">{{ $role->users_count }}</td>
                        <td class="px-6 py-4 text-center">
                            <span class="inline-flex items-center justify-center px-3 py-1 rounded-full text-xs font-medium bg-green-100 dark:bg-green-900 text-green-800 dark:text-green-200">Active</span>
                        </td>
                        <td class="px-6 py-4 text-sm text-center space-x-3">
                            <button type="button" 
                                    @click="openEdit(@js(['id' => $role->id, 'name' => $role->name, 'description' => $role->description]))"
                                    class="text-blue-600 hover:text-blue-900 dark:hover:text-blue-400">Edit</button>
                            <button type="button"
                                    @click="openPermissions(@js(['id' => $role->id, 'name' => $role->name, 'permissions' => $permissions[$role->name] ?? []]))"
    ...
    <div x-show="showPermissionMatrix" x-cloak class="fixed inset-0 z-50 overflow-y-auto" role="dialog" aria-modal="true">
        <div class="flex items-center justify-center min-h-screen px-4 pt-4 pb-20 text-center sm:block sm:p-0">
            <div x-show="showPermissionMatrix"
                 x-transition:enter="ease-out duration-300"
                 x-transition:enter-start="opacity-0"
                 x-transition:enter-end="opacity-100"
                 x-transition:leave="ease-in duration-200"
            ...
            <div x-show="showPermissionMatrix"
                 ...
                 x-transition:leave-start="opacity-100 translate-y-0 sm:scale-100"
                 x-transition:leave-end="opacity-0 translate-y-4 sm:translate-y-0 sm:scale-95"
                 class="inline-block align-bottom bg-white dark:bg-gray-800 rounded-2xl text-left overflow-hidden shadow-xl transform transition-all sm:align-middle sm:max-w-6xl sm:w-full">

                <div class="p-6 space-y-4">
                    <div class="flex items-center justify-between">
                        <div>
                            <h3 class="text-lg font-bold text-gray-900 dark:text-white">Permission Matrix</h3>
                            <p class="text-sm text-gray-600 dark:text-gray-400 mt-1">View what each role can access in the system</p>
                        </div>
                        <button type="button" @click="closePermissionMatrix()" class="text-gray-400 hover:text-gray-600 dark:hover:text-gray-300">✕</button>
                    </div>

                    <div class="max-h-[70vh] overflow-auto border border-gray-200 dark:border-gray-700 rounded-xl">
                        <table class="w-full text-sm">
                            <thead>
                                <tr class="border-b border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-900">
                                    <th class="px-4 py-3 text-left font-semibold text-gray-900 dark:text-white sticky left-0 bg-gray-50 dark:bg-gray-900">Module</th>
                                    @foreach($roles as $role)
                                    <th class="px-4 py-3 text-center font-semibold text-gray-900 dark:text-white whitespace-nowrap">{{ $role->name }}</th>
                                    @endforeach
                                </tr>
                            </thead>
                            <tbody class="divide-y divide-gray-200 dark:divide-gray-700">
                                ...
                                <tr class="hover:bg-gray-50 dark:hover:bg-gray-700/50">
                                    <td class="px-4 py-3 text-gray-900 dark:text-white sticky left-0 bg-white dark:bg-gray-800">{{ $moduleName }}</td>
                                    @foreach($roles as $role)
                    ...
                    <div class="flex justify-end pt-2">
                        <button type="button" @click="closePermissionMatrix()" class="px-4 py-2 rounded-lg bg-indigo-600 text-white hover:bg-indigo-700">Close</button>
                    </div>
                </div>
            </div>
        </div>
