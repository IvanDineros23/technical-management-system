...
<?php $__env->startSection('page-subtitle', 'Manage stock levels and inventory'); ?>
...
<div class="space-y-6" x-data="{ showAdd: false, showView: false, showEdit: false, showDelete: false, showRequests: false, selectedItem: null, formData: { name: '', sku: '', category: '', quantity: 0, unit: 'units', min_level: 0, notes: '' } }">
    ...
    <div class="grid grid-cols-1 gap-4 sm:grid-cols-3">
        <div class="bg-white dark:bg-gray-800 rounded-[20px] border border-gray-200 dark:border-gray-700 p-6">
            ...
            <p class="text-3xl font-bold text-gray-900 dark:text-white mt-2"><?php echo e($stats['totalItems'] ?? 0); ?></p>
        </div>
        <div class="bg-white dark:bg-gray-800 rounded-[20px] border border-gray-200 dark:border-gray-700 p-6">
            <p class="text-sm text-gray-600 dark:text-gray-400">Low Stock</p>
            <p class="text-3xl font-bold text-yellow-600 dark:text-yellow-400 mt-2"><?php echo e($stats['lowStock'] ?? 0); ?></p>
        </div>
        <div class="bg-white dark:bg-gray-800 rounded-[20px] border border-gray-200 dark:border-gray-700 p-6">
            <p class="text-sm text-gray-600 dark:text-gray-400">Out of Stock</p>
            <p class="text-3xl font-bold text-red-600 dark:text-red-400 mt-2"><?php echo e($stats['outOfStock'] ?? 0); ?></p>
        </div>
    </div>

    <!-- Filters -->
    <div class="bg-white dark:bg-gray-800 rounded-[20px] border border-gray-200 dark:border-gray-700 p-6">
        <form method="GET" action="<?php echo e(route('admin.inventory.index')); ?>" class="grid grid-cols-1 gap-4 sm:grid-cols-4">
            <div>
                <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">Search</label>
                <input type="text" name="search" value="<?php echo e(request('search')); ?>" placeholder="Search by name or SKU..." class="w-full px-4 py-2 border border-gray-300 dark:border-gray-600 rounded-lg dark:bg-gray-700 dark:text-white focus:ring-2 focus:ring-blue-500 focus:border-transparent">
            </div>
            <div>
            ...
            <div>
                ...
                <select name="status" class="w-full px-4 py-2 border border-gray-300 dark:border-gray-600 rounded-lg dark:bg-gray-700 dark:text-white focus:ring-2 focus:ring-blue-500 focus:border-transparent" onchange="this.form.submit()">
                    ...
                    <option value="low" <?php echo e(request('status') === 'low' ? 'selected' : ''); ?>>Low</option>
                    <option value="out" <?php echo e(request('status') === 'out' ? 'selected' : ''); ?>>Out</option>
                </select>
            </div>
            ...
            <table class="w-full">
                ...
                <tbody class="divide-y divide-gray-200 dark:divide-gray-700">
                    ...
                    <tr class="hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-colors">
                        <td class="px-6 py-4 text-sm font-medium text-gray-900 dark:text-white"><?php echo e($item->name); ?></td>
                        <td class="px-6 py-4 text-sm text-gray-600 dark:text-gray-400"><?php echo e($item->sku); ?></td>
                        <td class="px-6 py-4 text-sm text-gray-600 dark:text-gray-400"><?php echo e($item->category ?? 'N/A'); ?></td>
                        <td class="px-6 py-4 text-sm font-medium text-gray-900 dark:text-white"><?php echo e($item->quantity); ?> <?php echo e($item->unit); ?></td>
                        <td class="px-6 py-4 text-sm text-gray-600 dark:text-gray-400"><?php echo e($item->min_level); ?> <?php echo e($item->unit); ?></td>
                        <td class="px-6 py-4">
                        ...
                        <td class="px-6 py-4 text-sm">
                            <button class="text-blue-600 hover:text-blue-900 dark:hover:text-blue-400 mr-3" @click="selectedItem = <?php echo \Illuminate\Support\Js::from($item)->toHtml() ?>; showView = true">View</button>
                            <button class="text-orange-600 hover:text-orange-900 dark:hover:text-orange-400 mr-3" @click="selectedItem = <?php echo \Illuminate\Support\Js::from($item)->toHtml() ?>; formData = { name: selectedItem.name ?? '', sku: selectedItem.sku ?? '', category: selectedItem.category ?? '', quantity: selectedItem.quantity ?? 0, unit: selectedItem.unit ?? 'units', min_level: selectedItem.min_level ?? 0, notes: selectedItem.notes ?? '' }; showEdit = true">Edit</button>
                            <button class="text-red-600 hover:text-red-900 dark:hover:text-red-400" @click="selectedItem = <?php echo \Illuminate\Support\Js::from($item)->toHtml() ?>; showDelete = true">Delete</button>
                        </td>
                    </tr>
    ...
    <div x-show="showAdd" x-cloak class="fixed inset-0 z-50 overflow-y-auto" role="dialog" aria-modal="true" @keydown.escape.window="showAdd=false">
        <div class="flex items-center justify-center min-h-screen px-4 pt-4 pb-20 text-center sm:block sm:p-0">
            ...
            <div x-show="showAdd"
                ...
                <form method="POST" action="<?php echo e(route('admin.inventory.store')); ?>" class="p-6 space-y-4">
                    <?php echo csrf_field(); ?>
    ...
    <div x-show="showEdit" x-cloak class="fixed inset-0 z-50 overflow-y-auto" role="dialog" aria-modal="true" @keydown.escape.window="showEdit=false">
        <div class="flex items-center justify-center min-h-screen px-4 pt-4 pb-20 text-center sm:block sm:p-0">
            ...
            <div x-show="showEdit"
                 ...
                 x-transition:leave-end="opacity-0 translate-y-4 sm:translate-y-0 sm:scale-95"
                 class="inline-block align-bottom bg-white dark:bg-gray-800 rounded-2xl text-left overflow-hidden shadow-xl transform transition-all sm:align-middle sm:max-w-2xl sm:w-full border border-gray-200 dark:border-gray-700">
                <form method="POST" :action="selectedItem ? '<?php echo e(url('admin/inventory')); ?>/' + selectedItem.id : '#'" class="p-6 space-y-4">
                    <?php echo csrf_field(); ?>
    ...
    <div x-show="showDelete" x-cloak class="fixed inset-0 z-50 overflow-y-auto" role="dialog" aria-modal="true" @keydown.escape.window="showDelete=false">
        <div class="flex items-center justify-center min-h-screen px-4 pt-4 pb-20 text-center sm:block sm:p-0">
            ...
            <div x-show="showDelete"
                ...
                <form method="POST" :action="selectedItem ? '<?php echo e(url('admin/inventory')); ?>/' + selectedItem.id : '#'" class="p-6">
                    <?php echo csrf_field(); ?>
                ...
                <div class="p-6">
                    ...
                    <div class="overflow-x-auto">
                        <table class="w-full">
                            ...
                            <tbody class="divide-y divide-gray-200 dark:divide-gray-700">
                                <?php $__empty_1 = true; $__currentLoopData = $requests ?? []; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $inventoryRequest): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                                <tr class="hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-colors">
                                    <td class="px-4 py-3 text-sm font-medium text-gray-900 dark:text-white"><?php echo e($inventoryRequest->user->name ?? 'N/A'); ?></td>
                                    <td class="px-4 py-3 text-sm text-gray-600 dark:text-gray-400"><?php echo e($inventoryRequest->inventoryItem->name ?? 'N/A'); ?></td>
                                    <td class="px-4 py-3 text-sm text-gray-600 dark:text-gray-400"><?php echo e($inventoryRequest->quantity); ?> <?php echo e($inventoryRequest->inventoryItem->unit ?? 'pcs'); ?></td>
                                    <td class="px-4 py-3 text-sm text-gray-600 dark:text-gray-400"><?php echo e($inventoryRequest->purpose); ?></td>
                                    <td class="px-4 py-3 text-sm text-gray-600 dark:text-gray-400"><?php echo e($inventoryRequest->created_at->format('M d, Y')); ?></td>
                                    <td class="px-4 py-3">
                                        ...
                                        <?php elseif($inventoryRequest->status === 'approved'): ?>
                                        ...
                                        <?php elseif($inventoryRequest->status === 'rejected'): ?>
                                        ...
                                        <?php elseif($inventoryRequest->status === 'fulfilled'): ?>
                                    ...
                                    <td class="px-4 py-3 text-sm space-x-2">
                                        <?php if($inventoryRequest->status === 'pending'): ?>
                                        <form method="POST" action="<?php echo e(route('admin.inventory.requests.update', $inventoryRequest)); ?>" class="inline">
                                            <?php echo csrf_field(); ?>
                    ...
                    <div class="pt-4">
                        <?php echo e(($requests ?? null)?->links()); ?>
...
<?php echo $__env->make('layouts.dashboard', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH D:\GitHub\technical-management-system\technical-management-system\resources\views/admin/inventory.blade.php ENDPATH**/ ?>