...
<head>
    ...
    <title><?php echo $__env->yieldContent('title', 'Dashboard'); ?> - Gemarc Enterprises Inc</title>
    <link rel="icon" type="image/png" href="<?php echo e(asset('assets/0fae4580-eff0-4ee7-98e2-8ab80dd542cf-removebg-preview.png')); ?>">
...
<body class="h-full bg-gray-100 dark:bg-gray-900 overflow-hidden transition-colors" x-data="{ 
        ...
        <aside class="w-64 flex-shrink-0 bg-gradient-to-b from-blue-100 to-blue-50 dark:from-gray-800 dark:to-gray-900 rounded-r-3xl shadow-lg flex-col overflow-hidden border-r border-blue-200 dark:border-gray-700 fixed lg:relative h-full left-0 top-0 z-50 lg:z-auto" :class="{ 'hidden': !sidebarOpen && window.innerWidth < 1024, 'flex': true }" x-transition:enter="transition ease-out duration-300 lg:transition-none" x-transition:enter-start="-translate-x-full lg:translate-x-0" x-transition:enter-end="translate-x-0" x-transition:leave="transition ease-in duration-200 lg:transition-none" x-transition:leave-start="translate-x-0" x-transition:leave-end="-translate-x-full lg:translate-x-0">
            ...
            <div class="p-5 flex-shrink-0">
                <div class="flex flex-col items-center gap-3">
                    <img src="<?php echo e(asset('assets/0fae4580-eff0-4ee7-98e2-8ab80dd542cf-removebg-preview.png')); ?>" alt="Gemarc Logo" class="w-20 h-20 object-contain">
            ...
            <div class="px-4 py-3 border-t border-blue-200 dark:border-gray-700 flex-shrink-0">
                <a href="<?php echo e(route('profile.show')); ?>" class="flex items-center gap-2 mb-2 p-2 rounded-lg hover:bg-blue-200 dark:hover:bg-gray-700 transition-colors cursor-pointer group">
                    ...
                    <div class="flex-1 min-w-0">
                        <p class="text-xs font-medium text-gray-800 dark:text-white truncate group-hover:text-blue-700 dark:group-hover:text-blue-300"><?php echo e(auth()->user()->name); ?></p>
                        <p class="text-[10px] text-gray-500 dark:text-gray-400 truncate group-hover:text-gray-700 dark:group-hover:text-gray-300"><?php echo e(auth()->user()->role->name ?? 'User'); ?></p>
                    </div>
                </a>
        ...
        <div class="flex-1 flex flex-col min-h-0 overflow-hidden bg-gray-50 dark:bg-gray-800">
            ...
            <div class="lg:hidden h-16 bg-white dark:bg-gray-700 border-b border-gray-200 dark:border-gray-600 flex items-center px-4 gap-3 shadow-sm">
                ...
                <div class="flex-1 flex items-center gap-2">
                    <img src="<?php echo e(asset('assets/0fae4580-eff0-4ee7-98e2-8ab80dd542cf-removebg-preview.png')); ?>" alt="Gemarc Logo" class="w-8 h-8 object-contain">
    ...
    <?php if(session('status') || session('error') || session('warning') || session('info')): ?>
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            <?php if(session('status')): ?>
                ...
                window.dispatchEvent(new CustomEvent('toast', { 
                    detail: { message: <?php echo \Illuminate\Support\Js::from($statusToastMessage)->toHtml() ?>, type: 'success' } 
                }));
            <?php endif; ?>
            ...
            <?php if(session('warning')): ?>
                window.dispatchEvent(new CustomEvent('toast', { 
                    detail: { message: '<?php echo e(session('warning')); ?>', type: 'warning' } 
                }));
            <?php endif; ?>
            <?php if(session('info')): ?>
                window.dispatchEvent(new CustomEvent('toast', { 
                    detail: { message: '<?php echo e(session('info')); ?>', type: 'info' } 
                }));
            <?php endif; ?>
        });
    </script>
...
</html><?php /**PATH D:\GitHub\technical-management-system\technical-management-system\resources\views/layouts/dashboard.blade.php ENDPATH**/ ?>